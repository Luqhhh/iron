"""Frozen Depthwise-tree OOF comparison with independent replay."""
...
def initialize(root,output):
    if not output.is_relative_to(root/'local/runs/round2-v2.10'):
        raise ValueError('Fresh private V2.10 directory required')
    path=root/'configs/round2_v2_10/experiment.yaml'
    spec=yaml.safe_load(path.read_text())
    frozen=check(root,root/spec['source_run'])
    files=dict(frozen['files'])
    extras=[path,Path(__file__),Path(__file__).with_name('normalized_models.py'),root/spec['policy'],root/'uv.lock']
        ...
        for fold in range(5):
            ...
            for route,target in [(r,t) for r in spec['models'] for t in TARGETS]:
                ...
                if replay:
                    ...
                    model=NormalizedSnapshotRegressor(desired,target)
                ...
                if model.estimator_.tree_count_!=desired['iterations'] or model.actual_parameters_['grow_policy']!='Depthwise' or model.actual_parameters_['min_data_in_leaf']!=20:
                    raise ValueError('Depthwise execution mismatch')
                with threadpool_limits(limits=1):
...
def main():
    ...
    if a.action=='verify':
        ...
        try:
            compute(root,output)
            subprocess.run([sys.executable,'-m','bf_tap_r2.v2_depthwise','verify','--output',str(output)],cwd=root,check=True)
