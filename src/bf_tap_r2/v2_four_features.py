"""Frozen four-feature OOF comparison with independent replay."""
...
from .engineered_models import EngineeredRegressor, ENGINEERED_FIELDS, DERIVED_FEATURES
...
def initialize(root,output):
    if not output.is_relative_to(root/'local/runs/round2-v2.12'):
        raise ValueError('Fresh private V2.12 directory required')
    path=root/'configs/round2_v2_12/experiment.yaml'
    spec=yaml.safe_load(path.read_text())
    if tuple(spec['derived_features'])!=DERIVED_FEATURES:
        raise ValueError('Frozen derived feature pool mismatch')
    frozen=check(root,root/spec['source_run'])
    files=dict(frozen['files'])
    extras=[path,Path(__file__),Path(__file__).with_name('engineered_models.py'),root/spec['policy'],root/'uv.lock']
        ...
        for fold in range(5):
            ...
            for route,target in [(r,t) for r in spec['models'] for t in TARGETS]:
                ...
                if replay:
                    ...
                    if model.model_identity_!=identity or model.parameters!=desired or tuple(model.target_fields_)!=fields:
                        raise ValueError('Model identity/spec mismatch')
                else:
                    model=EngineeredRegressor(desired,target)
                    ...
                    write_json(path.with_suffix('.json'),{'model_sha256':digest(path),'parameters':model.actual_parameters_,
                        'target_scales':model.target_scales_.tolist()})
                ...
                np.testing.assert_array_equal(record['target_scales'],model.target_scales_)
                np.testing.assert_allclose(model.target_scales_,training[list(fields)].mean(),rtol=1e-14)
                if model.estimator_.tree_count_!=desired['iterations'] or model.actual_parameters_['grow_policy']!='SymmetricTree' or tuple(model.input_fields_)!=ENGINEERED_FIELDS or model.estimator_.feature_names_!=list(ENGINEERED_FIELDS):
                    raise ValueError('Engineered input/execution mismatch')
                with threadpool_limits(limits=1):
    ...
    for target in TARGETS:
        ...
        for route,d in tiers['decisions'][target].items():
            rows.append({'target':target,'candidate':route,'mean_wmape':d['mean_wmape'],'delta_current':-d['mean_gain'],
                ...
                'delta_N1':d['mean_wmape']-np.mean([metrics[target]['N1'][str(s)]['wmape'] for s in spec['split_seeds']]),
...
def main():
    ...
    if a.action=='verify':
        ...
        try:
            compute(root,output)
            subprocess.run([sys.executable,'-m','bf_tap_r2.v2_four_features','verify','--output',str(output)],cwd=root,check=True)
