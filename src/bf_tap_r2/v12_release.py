"""Candidate-specific V12 isolated iron release; requires explicit authorization."""
...
class IronView:
    """Expose only the first, iron output of an already fitted joint model."""
    def __init__(self, joint):
        self.joint=joint

    def predict(self, frame):
        prediction=self.joint.predict(frame)
        if prediction.shape!=(len(frame),2):
            raise ValueError('Joint iron/time outputs required')
        return prediction[:,0]


def check_authorization(release):
    a=release.get('authorization',{})
    if (a.get('user_requested_release') is not True
            or a.get('local_working_gate_exception')!='this_candidate_only'
            or a.get('desktop_write') is not True
            or a.get('uploads') is not False):
        raise ValueError('Explicit candidate-specific release authorization required')
    if (release['candidate']!='V12_IRON_JOINT_PLR001_A50'
            or release['target']!='tap_iron' or release['unchanged_target']!='tap_time_len'
            or release['blend_weight']!=.5 or release['recipe']!='joint_plr001'):
        raise ValueError('Frozen isolated V12 release identity mismatch')
...
def blended_payload(parent, ids, member, alpha):
    ...
    if values.shape != (len(ids),) or not np.isfinite(values).all():
        raise ValueError('Invalid iron member predictions')
    if not 0 <= alpha <= 1:
        raise ValueError('Invalid release blend weight')
    base = np.asarray([float(r['pred_tap_iron']) for r in rows])
    raw = (1-alpha)*base + alpha*values
    changed = np.maximum(raw, 0.)
    payload = payload_with_parent_other_column(ids, 'tap_iron', changed,
                                               [r['pred_tap_time_len'] for r in rows])
...
def verify_payload(payload, parent, ids, member, alpha):
    ...
    mismatch = sum(a['pred_tap_time_len'] != b['pred_tap_time_len'] for a, b in zip(rows, original))
    expected = np.maximum((1-alpha)*np.asarray([float(r['pred_tap_iron']) for r in original])
                          + alpha*np.asarray(member), 0.)
    actual = np.asarray([float(r['pred_tap_iron']) for r in rows])
    ...
    return {'rows': len(rows), 'unique_ids': len({r['sample_id'] for r in rows}),
            'template_order': True, 'time_string_mismatches': mismatch,
...
def save_and_cold(model, query, prediction, work, tolerance, parent=None, alpha=None):
    ...
    fitted=getattr(model,'joint',model)
    if hasattr(fitted,'optimizer_'):
        del fitted.optimizer_
    ...
    if parent is not None:
        with (work/'parent.csv').open('xb') as stream:
            stream.write(parent)
    subprocess.run([sys.executable, '-m', 'bf_tap_r2.v12_release', 'infer', '--work', str(work)], check=True)
...
def run(root, release_spec=Path('configs/round2_v12/RELEASE.yaml')):
    root = Path(root).resolve()
    release_path = (root/release_spec).resolve()
    if not release_path.is_relative_to(root/'configs/round2_v12'):
        raise ValueError('V12 release specification required')
    release = yaml.safe_load(release_path.read_text())
    check_authorization(release)
    ...
    if not out.is_relative_to(root/'local/runs/round2-v12-joint-tabm'):
        raise ValueError('Private V12 release output required')
    if desktop.exists():
    ...
    try:
        ...
        if (audit['status']!='passed' or audit['prediction_count']!=20
                or not all(confirm_summary['decisions'][label]['admitted'] for label in ['A35','CURRENT'])
                or not all(audit['comparisons'][label]['four_seed_pass'] for label in ['A35','CURRENT'])):
            raise ValueError('Frozen V12 four-seed confirmation required')
        dev_summary=json.loads((dev/'summary.json').read_text())
        chosen=next(r for r in dev_summary['records'] if r['target']=='tap_iron' and r['recipe']==release['recipe'])
        if set(chosen['comparisons']['A35']['alphas'].values())!={release['blend_weight']}:
            raise ValueError('Release weight must equal the frozen development choice')
        if file_hash(spec_path) != dev_manifest['spec_sha256']:
            raise ValueError('Historical V12 specification changed')
        for name, sha in dev_manifest['source_hashes'].items():
            if file_hash(root/'src/bf_tap_r2'/name) != sha:
                raise ValueError('Historical V12 source changed')
        for relative, sha in audit['hashes'].items():
            if file_hash(root/relative) != sha:
                raise ValueError('Historical V12 predictions changed')
        cm=json.loads((confirm/'manifest.json').read_text())
        if (cm['development_summary_sha256']!=file_hash(dev/'summary.json')
                or cm['development_audit_sha256']!=file_hash(dev/'audit-r1.json')
                or cm['confirmation_sha256']!=file_hash(root/'configs/round2_v12/CONFIRMATION.yaml')):
            raise ValueError('Confirmation parent evidence changed')
        for name,sha in cm['source_hashes'].items():
            if file_hash(root/'src/bf_tap_r2'/name)!=sha: raise ValueError('Confirmation source changed')
        for name,sha in cm['reference_hashes'].items():
            if file_hash(root/name)!=sha: raise ValueError('Confirmation reference changed')
        versions = {p: importlib.metadata.version(p) for p in spec['runtime_versions']}
        if versions != spec['runtime_versions']:
            raise ValueError('Frozen V12 runtime mismatch')
        ...
        model = JointRegressor(recipe, settings).fit(replay_train, replay_train[list(TARGETS)].values)
        replay = model.predict(train.loc[mask])
        expected = np.load(dev/f'joint-{release["recipe"]}-s{seed}-f{fold}.npy',allow_pickle=False)
        ...
        full = JointRegressor(recipe, settings).fit(train.reset_index(drop=True), train[list(TARGETS)].values)
        member = IronView(full).predict(test)
        cold = save_and_cold(IronView(full), test, member, out/'full',
        ...
        verification = {'G0': 'PASS', 'candidate': release['candidate'], 'readback': readback, 'clipping': clipping,
                         ...
                         'G1': {'platform_score': None, 'local_confirmation': audit['comparisons']},
                         'uploads': 0}
...
def main():
    ...
    parser.add_argument('--spec', type=Path, default=Path('configs/round2_v12/RELEASE.yaml'))
