...
def initialize(root,output):
    if not output.is_relative_to(root/'local/runs/round2-v2.8'):
        raise ValueError('Fresh private V2.8 directory required')
    path=root/'configs/round2_v2_8/experiment.yaml'
    ...
    for split in spec['split_seeds']:
        ...
        for fold in range(5):
            ...
            for route,target in [(r,t) for r in spec['models'] for t in TARGETS]:
                ...
                weights=numeric_weights(importance,spec['geometry']['numeric_uniform_floor']) if route=='KWT' else np.ones(len(FEATURES))
                ...
                if replay:
                    ...
                    model=GeometryRegressor(route,desired,target,weights)
                    model.model_identity_=identity
                    with threadpool_limits(limits=1):
                        fit_once(model,training,training[target],path,output,'cv',identity,40)
...
def main():
    ...
    if a.action=='verify':
        ...
        try:
            compute(root,output)
            subprocess.run([sys.executable,'-m','bf_tap_r2.v2_kernel_geometry','verify','--output',str(output)],cwd=root,check=True)
            write_json(output/'COMPLETE.json',{'status':'PASS','completed_cv_models':40,'total_fit_attempts':40,
