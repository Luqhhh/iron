"""Round2 V3.1 coarse runner with batch identity and identity-checked resume."""
...
from .v3_1_sampler import sample_s1
...
def _worker_run(payload: dict) -> dict:
    if _WORKER_TRAIN is None or _WORKER_FOLDS is None:
        raise RuntimeError("V3.1 worker not initialised")
    trial = payload["trial"]
...
def _code_version(root: Path) -> str:
    try:
        ...
        files = [root / "src/bf_tap_r2/v3_1_models.py", root / "src/bf_tap_r2/v3_1_sampler.py",
                 root / "src/bf_tap_r2/v3_1_run.py"]
...
def run_s1_coarse(root: Path, output: Path, *, sample_seed: int = 20260923,
                  fold_seed: int = 42, fold_ids: tuple[int, ...] = (0, 1),
                  workers: int = 16, limit: int | None = None) -> dict:
    private_root = (root / "local/runs/round2-v3.1-directed-search").resolve()
    if not output.resolve().is_relative_to(private_root):
        raise ValueError("V3.1 output must remain under the private V3.1 run root")
    output.mkdir(parents=True, exist_ok=True)
    spec = load_config(root)
    trials = sample_s1(root, seed=sample_seed)
    ...
    batch_id = f"v31-s1-coarse-{sample_seed}"
    identities = {}
    for trial in trials:
        identity = trial_identity(trial, batch_id=batch_id, data_hash=data_hash, fold_hash=fold_hash,
    ...
    for trial in trials:
        ...
        if old is not None:
            old_identity = json.dumps(old.get("identity"), sort_keys=True, default=str)
            desired_identity = json.dumps(identities[tid], sort_keys=True, default=str)
            if old_identity != desired_identity:
                raise ValueError(f"V3.1 cache identity mismatch for {tid}")
            continue
        pending.append(trial)
    results = []
    if pending:
        payloads = [{"trial": trial, "identity": identities[trial["trial_id"]],
                     "fold_ids": list(fold_ids), "inner_seed": 9000 + i}
    ...
    return {"batch_id": batch_id, "trials": len(trials), "pending_at_start": len(pending),
            ...
            "data_hash": data_hash, "fold_hash": fold_hash, "code_version": code_version,
            "agent_uploads": 0}
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps(run_s1_coarse(args.root, args.output, sample_seed=args.sample_seed,
