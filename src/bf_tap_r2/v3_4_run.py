"""Round2 V3.4 append-only runner.

The runner writes only under the private V3.4 run directory.  It never creates
a submission, never uploads a package, and never overwrites an existing run
record.  Every trial identity includes the source/config digest, data/fold
hashes, explicit EBM bag protocol hash where applicable, and a canonical trial
hash, so dirty source changes or repaired configurations cannot silently reuse
...
from .v3_4_models import evaluate_v34_outer_folds
from .v3_4_sampler import load_v34_config, sample_v34
...
def _worker_run(payload: dict) -> dict:
    if _WORKER_TRAIN is None or _WORKER_FOLDS is None:
        raise RuntimeError("V3.4 worker was not initialised")
    result = evaluate_v34_outer_folds(_WORKER_TRAIN, _WORKER_FOLDS, payload["trial"],
...
def _hash_folds(folds: np.ndarray, fold_ids: Sequence[int]) -> str:
    payload = np.asarray([int(v) for v in fold_ids], dtype=np.int64).tobytes()
    payload += np.ascontiguousarray(folds, dtype=np.int64).tobytes()
    return _sha256_bytes(payload)


def _source_files(root: Path) -> list[Path]:
    names = [
        "configs/round2_v3_4/search.yaml",
        "src/bf_tap_r2/v3_4_bags.py",
        "src/bf_tap_r2/v3_4_models.py",
        "src/bf_tap_r2/v3_4_sampler.py",
        "src/bf_tap_r2/v3_4_run.py",
        "src/bf_tap_r2/v3_4_composition.py",
        "src/bf_tap_r2/v3_1_models.py",
        "src/bf_tap_r2/v3_1_fusion.py",
        "src/bf_tap_r2/v3_local_search.py",
    ]
...
def _bag_hash_for_trial(trial: Mapping[str, Any], train: pd.DataFrame) -> str | None:
    ...
    if kind == "ebm_residual":
        ...
        protocol = dict(base.get("protocol", {}))
        return protocol_bag_hash(
            train,
            n_outer_bags=int(base_params.get("outer_bags", 4)),
            n_inner_splits=int(params.get("inner_splits", 5)),
            seed=int(protocol.get("bag_seed", base_params.get("random_state", 42))),
        )
    return None


def v34_trial_identity(trial: Mapping[str, Any], *, batch_id: str, data_hash: str,
    ...
    base.update({
        ...
        "training_protocol": str(trial.get("protocol", {}).get("bags", "catboost-or-shrink")),
...
def _private_output(root: Path, output: Path) -> None:
    private = (root / "local/runs/round2-v3.4-ebm-and-constrained-composition").resolve()
    if not output.resolve().is_relative_to(private):
        raise ValueError("V3.4 output must remain private under local/runs/round2-v3.4-ebm-and-constrained-composition")
...
def run_batch(root: Path, output: Path, *, stage: str, fold_seed: int,
    ...
    if not trials:
        raise ValueError("No V3.4 trials selected")
    ...
    for trial in trials:
        ...
        identity = v34_trial_identity(
            trial,
            batch_id=f"v34-{stage}-seed{fold_seed}",
            data_hash=data_hash,
            fold_hash=fold_hash,
        ...
        if old is not None:
            if not _identity_matches(old.get("identity", {}), identity):
                raise ValueError(f"V3.4 cache identity mismatch: {trial['trial_id']}")
            continue
    ...
    (output / "batch.json").write_text(json.dumps(batch_manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    results: list[dict] = []
    if pending:
        payloads = [{"trial": trial, "identity": identities[trial["trial_id"]],
                     "fold_ids": [int(v) for v in fold_ids]} for trial in pending]
        with ProcessPoolExecutor(max_workers=max(1, int(workers)),
                                 initializer=_init_worker, initargs=(train, folds)) as executor:
...
def select_refine_trials(coarse_records: Sequence[dict], max_per_target: int = 10) -> list[dict]:
    """Deterministic shortlist from coarse records.

    This is a mechanical shortlist, not the final diversity/simple-mix policy
    in the task book.  The final freeze file can override it, but any override
    ...
    for target in TARGETS:
        ...
        selected.extend(rows[: max(0, int(max_per_target))])
    return selected


def _json_trial(record: Mapping[str, Any]) -> dict:
    trial = record.get("trial")
    if not isinstance(trial, Mapping):
        raise ValueError("Record lacks full trial specification")
    return dict(trial)


def run_refine(root: Path, coarse_ledger: Path, output: Path, *,
               seeds: Sequence[int] = (42, 3407), workers: int = 16,
               max_per_target: int = 10) -> dict:
    _private_output(root, output)
    output.mkdir(parents=True, exist_ok=True)
    coarse = _read_records(coarse_ledger)
    selected = select_refine_trials(coarse, max_per_target=max_per_target)
    if not selected:
        raise ValueError("No coarse records available for V3.4 refine")
    summary = {"refine_selected": len(selected), "seeds": [int(s) for s in seeds],
               "completed_per_seed": {}, "agent_uploads": 0}
    for seed in seeds:
...
def conditional_extension_trigger(refine_records: Sequence[dict], *,
                                  baseline_wmape_by_target: Mapping[str, float],
                                  trigger_delta: float = 0.02,
                                  positive_splits_required: bool = True) -> dict:
    """Pure budget trigger for the optional 16 V3.4 extension slots."""
    ...
    for target, rows in by_target.items():
        if target not in baseline_wmape_by_target:
            raise ValueError(f"Missing baseline WMAPE for {target}")
        baseline = float(baseline_wmape_by_target[target])
        deltas = []
        for row in rows:
            pooled = float(row.get("pooled_wmape", float("inf")))
            folds = row.get("fold_scores", {})
            positive = sum(float(v) < baseline for v in folds.values()) if folds else 0
            delta = baseline - pooled
            deltas.append({"trial_id": row.get("trial_id"), "delta": delta,
                           "positive_folds": int(positive), "n_folds": len(folds)})
        best = sorted(deltas, key=lambda row: (-row["delta"], str(row["trial_id"])))
        qualifies = bool(best and best[0]["delta"] >= float(trigger_delta)
                         and (not positive_splits_required or best[0]["positive_folds"] >= best[0]["n_folds"]))
        targets[target] = {"baseline_wmape": baseline, "best": best[:5], "qualifies": qualifies}
        triggered = triggered or qualifies
    return {
        "triggered": bool(triggered),
        "trigger_delta": float(trigger_delta),
        "positive_splits_required": bool(positive_splits_required),
        "targets": targets,
        "max_new_slots": 16,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    coarse.add_argument("--workers", type=int, default=16)
    ...
    refine.add_argument("--max-per-target", type=int, default=10)
    args = parser.parse_args(argv)
    root = Path.cwd()
    if args.command == "coarse":
        summary = run_batch(root, args.output, stage="coarse", fold_seed=int(args.fold_seed),
                            fold_ids=(0, 1), workers=int(args.workers), limit=args.limit)
    elif args.command == "refine":
        summary = run_refine(root, args.coarse_ledger, args.output,
                             workers=int(args.workers), max_per_target=int(args.max_per_target))
