...
**optimization-v0.32** 已完成两个目标隔离的同铁口 OOB 响应条件化：固定 256 棵森林、v0.29 selected members、每树等权和原后处理，仅在原成员内优先使用同铁口响应，空交集保留原集合。0 个新增 fit，14 份既有 OOB 附件与 14 份按 ID 对齐的铁口向量完成独立恢复；A/B 最终分别只改铁量 321 行、只改时长 298 行。A/B 相对 V30A 的历史 J ΔE 为 `+0.00006051/-0.00018334`，两者仍劣于 V1。两份 335 行包已同时冻结，7 个 cutoff 的全量/反序/分块/子集/单行冷审计通过；随后按用户显式要求以不覆盖模式写入桌面并核验摘要/payload。平台预算 2/2 尚未使用，agent 上传 0，V30A=83.3175 仍是最高用户回传原包。[v0.32 结果](docs/optimization_v0_32/RESULTS.md)
...
| 项目 | 当前状态 |
| --- | --- |
| v0.32 双候选 G0/G1 | G0 PASS；A/B 相对 V30A 的历史 J ΔE +0.00006051/-0.00018334，目标隔离最大残差 5.55e-17；两包冻结、平台反馈待用户回传，预算 0/2，agent 上传 0 |
...
| 阶段 | 结果 | 决策 |
| --- | --- | --- |
| [v0.32](docs/optimization_v0_32/RESULTS.md) | 0 fit；原 OOB members 内同铁口优先；A/B 相对 V30A 的 ΔJ +0.00006051/-0.00018334；最终改动 321 铁量行/298 时长行 | G0 PASS；两包与冷审计已冻结，预算待用户使用 0/2；保留 V30A 原包，等待反馈 |
