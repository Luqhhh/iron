...
V2.1 C2增量优化本地阶段已完成：两个目标均选B3（三训练种子等权），两种切分平均铁量/时长WMAPE为3.959302%/4.048214%，原C2为3.991918%/4.075389%。80次CV＋4次全量真实拟合、9项专用测试及独立冷推理完成，原96.0537包保持不变。A/B已按目标隔离放入桌面 `submission/round2-v2.1/`，等待用户自行上传回传；只有A/B都正收益后才生成C。[V2.1完整结果](docs/round2_v2_1/RESULTS.md)
