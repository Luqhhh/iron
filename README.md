...
## 当前最新状态（2026-09-25，Round2 V4.1）

- 当前最高用户回传为 **V36 = 96.2734**，距严格大于 96.3 还差 0.0266；未独立登录平台验证。
- V4.1 以公开可重建 V34 EBM 为锚点，完成 24 个交叉拟合残差候选粗筛；7 个时长候选进入两 seed 完整五折，铁量未达继续门槛。
- 最优 `HGB_L2_L15_A050` 将时长 WMAPE 从 0.04015362 降到 0.03963951，相对该锚点的平均完整包贡献 **+0.02570552**，两 seed 均正、9/10 折改善、两铁口均改善，候选分层为 formal。
- 本机器没有 V34_A/V36 私有 OOF/模型/test 缓存，因此该收益不能直接叠加到 96.2734。用户随后显式请求生成公开锚点探索包；其同口径两 seed 五折本地分为 **96.119985**，相对公开锚点 +0.025706，但低于 V36 开发分 96.203763。ZIP 私有交付、未上传，详见 [V4.1 结果](docs/round2_v4_1/RESULTS.md)。

以下 V3.5 与更早段落保留为阶段历史。

## 历史状态（2026-09-24，Round2 V3.5）
