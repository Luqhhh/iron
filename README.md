...
**optimization-v0.31** 已完成两个目标隔离的 OOB 叶响应 occurrence pooling 工程与离线评价：0 个新模型/树/预处理/校准 fit，复用 14 份已认证 v0.29 OOB 附件；A 相对 V30A 的 H1/J ΔE 为 +0.00000149/+0.00002701，B 为 +0.00003565/-0.00008593，均未超过 V1。两包在任何本轮平台反馈前同时冻结，冷审计全量/反序/分块/子集/单行通过且旧等树质量 switch-back 精确复现；平台预算 2、agent 上传 0，默认 A→B 各一次，待用户显式执行。[v0.31 结果](docs/optimization_v0_31/RESULTS.md)
