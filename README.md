...
## 团队研究快照与策略复核（2026-09-26，09-27更新）

- `round2_current_platform_best` 登记为用户回传 **A35 = 96.3366**；当前目标 **96.4**，尚差 **0.0634**。
- V7 时长和 V12 铁量单包均已交付、平台回传未登记。V12 相对 A35 本地平均 +0.016088，相对 V9 再增加 +0.011627；V13 修复对照40+1拟合完成，但对V9/V12铁量无增量、对V7时长略负，无确认候选。
- 本次源码反例确认跨 seed 选权仍使用同一批评估标签，需要补充样本级嵌套验证；四 seed 结果保留为切分稳定性证据。仅输入的来源分类 AUC 为 0.481846/0.520041，未检出明显分布偏移。
- 新增样本级嵌套执行核心：在外层训练样本内重新拟合OOF、共同选择候选/权重，评估标签仅在事后评分接口出现；29项新测试和合成扰动验证通过，尚未接入真实神经实验。优先补V7/V12反馈、完成适配器与账本集成，不追加已闭合PLE单目标网格。见 [后续研究与实现](docs/team_strategy_20260926/FOLLOWUP_20260927.md)；[初次策略复核](docs/team_strategy_20260926/REVIEW.md)保留历史建议及限制。

以下各阶段的“当前”表述保留当时含义；读取最新复赛状态请使用上面的带日期入口及 `EVIDENCE_STATUS.json -> round2_current_platform_best`。

## 当前最新状态（2026-09-24，Round2 V3.5）

- 平台最佳仍为用户回传的 **V34_A = 96.2684**，其次 **V34_B = 96.2660**；两项均未独立账号核验。
- V3.5 固定 **256** 槽位、每目标 **12** 项完整开发精筛和 S0 cap 检查已完成；无 N/A、无 duplicate、无 failed。
- 开发组合候选（seeds 42/3407 选权，非最终 outer）包分 **96.2017**，同开发覆盖 L1 为 **96.1510**；未达到本地工作门槛 **96.25**。
- S0 放宽 cap 只带来可忽略的 WMAPE 变化，保留原 `cap=0.50`；条件追加未触发，最终 outer seed 23003 未消费。
- 本轮新平台包 0、上传 0；不宣称平台 >96.3 已完成。详见 [V3.5 结果](docs/round2_v3_5/RESULTS.md)。

## 当前最新状态（2026-09-23，Round2 V3）

- 当前平台首选仍为用户回传的 **AJ3 铁量 ＋ 完整 B3 时长：96.1259**；对应本地完整包参照 **96.0123883047359**。
- Round2 V3 首轮 **400/400** 项粗筛已完成，其中 `xgboost==2.1.4` 已作为 `round2` 可选依赖安装并完成 40 项 XGBoost 搜索。
- CatBoost 精筛与嵌套融合在 seeds 42/3407 上得到完整包本地约 **96.100735**，相对 AJ3 本地参照 **+0.088347**，进入 `candidate_pool`。
- 在 seeds 42/3407 上冻结外层成员和权重后，用独立 seed 2026 复验：完整包本地 **96.116174**，相对同 seed AJ3 参照 **+0.088335**，原始 +0.088 收益保留。
- XGBoost 精筛最佳为铁量 0.04203、时长 0.04659，未进入最优融合；证据保留，不关闭路线。
- 当前没有生成 V3 平台包，也没有上传；下一步若继续，需先明确授权后再封包和冷审计。
- 详见 [V3 计划](docs/round2_v3/PLAN.md)、[V3 结果](docs/round2_v3/RESULTS.md)、[文档索引](docs/INDEX.md)。

以下 V2.x/V0.x 段落保留为阶段历史；其中“当前平台首选”等表述不适用于 2026-09-23 之后的 V3 阶段。

V2.2本地评估完成：以当前用户回传96.0982的C2铁量＋完整B3时长包为基准，零新增CV评估I1/I2/T1。铁量选I2（C2/D4等权），时长T1也通过门槛；按用户确认的单名额及冻结排序，仅交付 `V22_I_ONLY`，时长字符串原样保留。定向5项、完整锁定640项测试通过，实际只新增1次全量D4铁量训练。新包待用户回传，当前平台首选仍为96.0982；不重复询问上传方式/额度。[V2.2完整结果](docs/round2_v2_2/RESULTS.md)

V2.1 C2增量优化本地阶段已完成：两个目标均选B3（三训练种子等权），两种切分平均铁量/时长WMAPE为3.959302%/4.048214%，原C2为3.991918%/4.075389%。80次CV＋4次全量真实拟合、9项专用测试及独立冷推理完成，原96.0537包保持不变。用户按A→B回传 **96.0448 / 96.0982**，相对原C2分别为−0.0089 / +0.0445分。当前首选为 **B：原C2铁量＋B3时长，96.0982**；A关闭，不生成C。分数未独立登录平台核验，原包与A/B原件保留。[V2.1完整结果](docs/round2_v2_1/RESULTS.md)

V2 泄露审计已完成：未发现直接训练/测试重复或标签混入输入的证据；打乱标签后 C2 精度退回基线附近，遮蔽 pig 后仍有较强预测能力。生产时点与官方生成器/切分来源仍无法核实。[审计范围、对照与限制](docs/round2_v2_leakage_audit.md)
...
V2 的 11 条旧复赛路线已重新比较，两个目标均选择 C2（CatBoost RMSE）。两种子平均 OOF J 为 **4.033654%**（M0 为 17.694999%），用户于 2026-09-22 回传 V2 平台成绩：**M0=82.7733、C2=96.0537，C2 提升 13.2804 分**；尚未独立登录平台核验。140 次 CV 与 2 次全量拟合、66 项锁定测试及独立模型回读已完成，C2 新包已写入桌面 submission，M0 已备份。[完整对比](docs/round2_v2_comparison.md)
...
**optimization-v0.32** 已完成两个目标隔离的同铁口 OOB 响应条件化：固定 256 棵森林、v0.29 selected members、每树等权和原后处理，仅在原成员内优先使用同铁口响应，空交集保留原集合。0 个新增 fit，14 份既有 OOB 附件与 14 份按 ID 对齐的铁口向量完成独立恢复；A/B 最终分别只改铁量 321 行、只改时长 298 行。A/B 相对 V30A 的历史 J ΔE 为 `+0.00006051/-0.00018334`。两包经冷审计并写入桌面后，用户按 A→B 回传 **83.2764/83.0910**，分别低于 V30A 0.0411/0.2265；两项关闭，不生成第三个组合，V30A=83.3175 仍是最高用户回传原包。平台预算 2/2，agent 上传 0；分支已按用户要求推送。[v0.32 结果](docs/optimization_v0_32/RESULTS.md)

**optimization-v0.31** 已完成两个目标隔离的 OOB 叶响应 occurrence pooling 工程与离线评价：0 个新模型/树/预处理/校准 fit，复用 14 份已认证 v0.29 OOB 附件；两包在任何本轮平台反馈前同时冻结，冷审计全量/反序/分块/子集/单行通过且旧等树质量 switch-back 精确复现。用户随后按 A→B 各提交一次并回传 I/V31I=83.3123、T/V31T=83.3116，分别比父包 V30A=83.3175 低 0.0052、0.0059，也低于 V29T=83.3141；两项关闭，V30A 仍是当前最高用户回传完整包。平台预算 2/2，agent 上传 0。[v0.31 结果](docs/optimization_v0_31/RESULTS.md)

**optimization-v0.30** 已完成双目标 OOB 收益组合与时长森林固定扩容。A 逐字符串复制 V29I 铁量列与 V29T 时长列（不平均、不重算、不再门控）；B 把已认证 V26A 256 棵绝对误差时长森林深复制后 `warm_start=True` 固定追加到 1024 棵，沿用 v0.29 逐树 OOB 协议后重放原 gate 与 V21 收缩。轮内恰好 7 次真实追加 fit、5,376 棵新树（r3/r4 的 2 个已完成拟合被显式恢复，0 次 re-fit），其余模型/预处理器/校准拟合为 0；A 相对 V28I 的增量与 V29I、V29T 各自增量之和最大残差 5.55e-17，B 相对 A 的 H1/J ΔE 为 -0.00004835/-0.00004136，两项仍明显劣于 V1。两份 335 行包在任何本轮反馈前同时冻结，独立冷审计通过；根 Python 3.12.12 为 484 passed，独立 worker 86 passed。用户按 A→B 各提交一次，回传 A=83.3175（晋级为当前最高）、B=83.2654（关闭）；agent 上传 0。[v0.30 结果](docs/optimization_v0_30/RESULTS.md)
...
| 项目 | 当前状态 |
| --- | --- |
| v0.32 双候选 G0/G1 | G0 PASS；A/B 相对 V30A 的历史 J ΔE +0.00006051/-0.00018334，目标隔离最大残差 5.55e-17；平台用户回传 83.2764/83.0910，均关闭并保留 V30A，预算 2/2，agent 上传 0 |
| v0.32 锁定测试 | 根 Python 3.12.12：507 passed；v0.32 worker：41 passed；A/B × 7 cutoff 冷审计 full/reverse/chunk/subset/single 与 ZIP 回读 PASS，所有 fit 尝试 0 |
| v0.32 桌面交付 | A/B 两份候选名 ZIP 以不覆盖模式写入 `C:\Users\lqh22\Desktop`，SHA-256 与冻结源包一致且 payload 逐字节相同；未删除旧文件，agent 上传 0 |
| v0.30 双候选 G0/G1 | G0 PASS；A 列组合恒等式残差 ≤5.55e-17；B 相对 A 的 H1/J ΔE -0.00004835/-0.00004136，B/A 相对 V1 的 J 为 +0.00396392/+0.00400528；7 次追加 fit、5,376 棵新树（含 2 个恢复拟合）、0 其他拟合；平台用户回传 A=83.3175（晋级，与 83.3175 加性推算一致）、B=83.2654（关闭），预算 2/2，agent 上传 0 |
| v0.30 锁定测试 | 根 Python 3.12.12：484 passed；独立 worker：86 passed（v0.15 34、v0.24 1、v0.25 1、v0.26 5、v0.27 8、v0.29 20、v0.30 17）；冷审计全量/反序/分块/子集/单样本与 ZIP 回读 PASS，fit 尝试 0 |
| v0.30 桌面交付 | 按用户显式指令替换桌面提交包：旧 iron 包已不存在（检索 0），A/B 两份 ZIP 写入 `C:\Users\lqh22\Desktop` 并核验摘要/成员/335 行；private local 原包保留，agent 上传 0 |
...
| 阶段 | 结果 | 决策 |
| --- | --- | --- |
| [v0.32](docs/optimization_v0_32/RESULTS.md) | 0 fit；原 OOB members 内同铁口优先；A/B 相对 V30A 的 ΔJ +0.00006051/-0.00018334；平台回传 83.2764/83.0910 | G0 PASS；预算 2/2，两项关闭，不生成第三个组合；保留 V30A |
| [v0.31](docs/optimization_v0_31/RESULTS.md) | 0 fit；occurrence pooling；平台回传 83.3123/83.3116 | 两项低于 V30A，关闭并保留 V30A |
| [v0.30](docs/optimization_v0_30/RESULTS.md) | A 合并 V29I/V29T；B 固定扩容时长森林到 1024；平台回传 83.3175/83.2654 | A 晋级为当前最高用户回传完整包；B 关闭 |
...
```bash
uv sync --locked --extra dev --extra round2 --python 3.12
