...
**optimization-v0.30** 已完成双目标 OOB 收益组合与时长森林固定扩容。A 逐字符串复制 V29I 铁量列与 V29T 时长列（不平均、不重算、不再门控）；B 把已认证 V26A 256 棵绝对误差时长森林深复制后 `warm_start=True` 固定追加到 1024 棵，沿用 v0.29 逐树 OOB 协议后重放原 gate 与 V21 收缩。轮内恰好 7 次真实追加 fit、5,376 棵新树（r3/r4 的 2 个已完成拟合被显式恢复，0 次 re-fit），其余模型/预处理器/校准拟合为 0；A 相对 V28I 的增量与 V29I、V29T 各自增量之和最大残差 5.55e-17，B 相对 A 的 H1/J ΔE 为 -0.00004835/-0.00004136，两项仍明显劣于 V1。两份 335 行包在任何本轮反馈前同时冻结，独立冷审计通过；根 Python 3.12.12 为 484 passed，独立 worker 86 passed。平台预算 2、实际上传 0，默认 A→B。[v0.30 结果](docs/optimization_v0_30/RESULTS.md)
...
| 项目 | 当前状态 |
| --- | --- |
| v0.30 双候选 G0/G1 | G0 PASS；A 列组合恒等式残差 ≤5.55e-17；B 相对 A 的 H1/J ΔE -0.00004835/-0.00004136，B/A 相对 V1 的 J 为 +0.00396392/+0.00400528；7 次追加 fit、5,376 棵新树（含 2 个恢复拟合）、0 其他拟合；两份包已冻结，平台预算 2、上传 0 |
