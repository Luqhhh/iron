"""Build a label-free exploratory challenger from the frozen V10 result.

The challenger leaves iron unchanged and moves only a pre-declared low-support
subset of QRF time predictions 25% toward the train-only, same-spout 60-day
median.  The rule was fixed before reading any test target (there are no test
target columns in the inputs).  It is deliberately separate from the frozen
V10 reconstruction and never overwrites that package.
"""
...
def recent_spout_medians(test: pd.DataFrame, history: pd.DataFrame) -> np.ndarray:
    ...
    for row in test.itertuples(index=False):
        mask = (
            history.spout_no.astype(str).to_numpy() == str(row.spout_no)
...
def build(output: Path) -> dict:
    ...
    archive = submission / "Luqhhh_bf_tap_predict_prelim_TGATE.zip"
    ...
    if hashlib.sha256(payload).hexdigest() != sha256(csv_path):
        raise ValueError("ZIP payload differs from result.csv")

    summary = {
        "status": "PASS_READY_TO_SUBMIT_EXPLORATORY",
        "candidate": "T_GATE_V10_QRF_MEDIAN_RECENT60_SPOUT",
        "stage": "test_a",
        "rows": ROWS,
