"""Zero-fit independent arithmetic and identity audit for V14 development."""
import argparse
import hashlib
...
from bf_tap_r2.v14_joint_ple import load_references
import torch
...
def audit(root, out, destination):
    spec_path=root/'configs/round2_v14/SPEC.yaml'
    ...
    bykey={e['key']:e for e in events}; predictions={}; checked={}; prep_cache={}
    for name in spec['recipes']:
        ...
        for s,fv in folds.items():
            for f in range(5):
                ...
                assert len(meta['bin_trace'])==2 and meta['activation']==spec['recipes'][name]['activation']
                if spec['recipes'][name]['activation']:
                    assert meta['nonlinear_weights_nonzero']==0 and meta['nonlinear_weight_norm']==0
                else:
                    assert meta['nonlinear_weights_nonzero']>0 and meta['nonlinear_weight_norm']>0
                for stage,(trace,part) in enumerate(zip(meta['bin_trace'],[inner,outer])):
                    cache_key=(s,f,stage)
                    if cache_key not in prep_cache:
                        ...
                        prep_cache[cache_key]={'standardized_input_sha256':hashlib.sha256(numeric.tobytes()).hexdigest(),
                                              'bins':[b.tolist() for b in bins]}
                    assert trace['rows']==len(part) and trace['ids_digest']==digest(part.sample_id.tolist())
                    for k,v in prep_cache[cache_key].items():assert trace[k]==v
    ...
    for row in summary['records']:
        ...
        for label,ref in [('A35',base),('Q20',q20),('CURRENT',current)]:
            ...
            for held,fv in folds.items():
                ...
                check(row['standard_control_wmape'][str(held)],err(y,controls[target][name][held]))
    ...
    result={'status':'passed','prediction_count':len(checked),'hashes':checked,'selected_confirmation':selected,'model_fits':0,
            ...
            'audit_script_sha256':file_hash(__file__),'preprocessing_reconstructions':len(prep_cache),'epoch_cap_hits':sum(e['metadata']['budget_limited'] for e in events)}
...
if __name__=='__main__':
    ...
    assert args.run.resolve().is_relative_to(root/'local/runs/round2-v14-joint-ple')
