"""Zero-fit independent arithmetic and identity audit for V16 development."""
import argparse
import hashlib
...
from bf_tap_r2.v16_sequential_masks import load_references, implementation_hashes
...
def audit(root, out, destination):
    spec_path=root/'configs/round2_v16/SPEC.yaml'
    spec=yaml.safe_load(spec_path.read_text())
    manifest=json.loads((out/'manifest.json').read_text())
    summary=json.loads((out/'summary.json').read_text())
    assert file_hash(spec_path)==manifest['spec_sha256']
    assert implementation_hashes()==spec['implementation_hashes']==manifest['implementation_hashes']
    ...
    for seed in folds:
        np.testing.assert_array_equal(platform[seed]['tap_iron'],current[seed]['tap_iron'])
        np.testing.assert_array_equal(platform[seed]['tap_time_len'],base[seed]['tap_time_len'])
        np.testing.assert_array_equal(a60[seed]['tap_iron'],base[seed]['tap_iron'])
        expected=q20[seed]['tap_time_len']+(spec['platform_reference_time_alpha']-.2)/(.35-.2)*(base[seed]['tap_time_len']-q20[seed]['tap_time_len'])
        np.testing.assert_allclose(a60[seed]['tap_time_len'],expected,rtol=0,atol=1e-10)
    control=json.loads((out/'control.json').read_text()); c=spec['control']
    assert control['sha256']==file_hash(out/'control.npy')
    np.testing.assert_array_equal(np.load(out/'control.npy'), joint[c['split_seed']][folds[c['split_seed']]==c['fold']])
    ...
    for name in spec['recipes']:
        ...
        for s,fv in folds.items():
            for f in range(5):
                ...
                diagnostics=meta['mask_diagnostics']; learned=spec['recipes'][name]['learned_masks']
                assert diagnostics['learned']==learned
                dimension=len(FEATURES)+len(set(outer.spout_no.astype(int)))+1
                assert diagnostics['shape']==[spec['tabnet_policy']['n_steps'],len(outer),dimension]
                assert 0<=diagnostics['minimum']<=diagnostics['maximum']<=1+1e-6
                assert diagnostics['simplex_max_error']<1e-5
                assert len(diagnostics['nonzero_fraction_by_step'])==spec['tabnet_policy']['n_steps']
                assert all(0<v<=1 for v in diagnostics['nonzero_fraction_by_step'])
                assert len(diagnostics['mean_entropy_by_step'])==spec['tabnet_policy']['n_steps']
                assert all(0<=v<=np.log(dimension)+1e-5 for v in diagnostics['mean_entropy_by_step'])
                assert np.isfinite(diagnostics['attention_gradient_norm']) and diagnostics['attention_gradient_norm']>=0
                assert 0<=diagnostics['maximum_row_std']<=1
                assert meta['parameter_count']>0
                if not learned:
                    assert diagnostics['attention_gradient_norm']==diagnostics['maximum_row_std']==0
                    assert diagnostics['nonzero_fraction_by_step']==[1.]*spec['tabnet_policy']['n_steps']
                    np.testing.assert_allclose(diagnostics['mean_entropy_by_step'],np.log(dimension),rtol=0,atol=1e-5)
    ...
    for row in summary['records']:
        ...
        for label,ref in [('V12_PLATFORM',platform),('A60',a60),('A35',base),('Q20',q20),('CURRENT',current)]:
            ...
            for held,fv in folds.items():
                ...
                if label in ('V12_PLATFORM','CURRENT') and g<=0: ok=False
                if label=='CURRENT':
                    metrics[target][name][str(held)]=details(y,blend,fv,frame.spout_no.values)
                    other=next(x for x in TARGETS if x!=target)
                    check(row['development_package_scores'][str(held)],100-50*(err(y,blend)+err(frame[other].values,platform[held][other])))
                check(row['single_wmape'][str(held)],err(y,p[held]))
                check(row['joint_periodic_control_wmape'][str(held)],err(y,joint[held][:,list(TARGETS).index(target)]))
...
if __name__=='__main__':
    ...
    assert args.run.resolve().is_relative_to(root/'local/runs/round2-v16-sequential-masks')
