"""Build a controlled follow-up to TGATE using a wider support gate.

This is intentionally a zero-fit, label-free challenger.  It starts from the
frozen V10 submission, leaves iron unchanged, and applies the same 25% recent
same-spout median shrink as TGATE, but extends the QRF low-support gate from
effective_neighbors < 500 to < 600.  The wider gate is isolated as the only
change so the next platform submission has a clear interpretation.
"""
...
def build(output: Path) -> dict:
    ...
    archive = submission / "Luqhhh_bf_tap_predict_prelim_TGATE600.zip"
    ...
    summary = {
        ...
        "candidate": "T_GATE600_V10_QRF_MEDIAN_RECENT60_SPOUT",
        "stage": "test_a",
        "rows": ROWS,
