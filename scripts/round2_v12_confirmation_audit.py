"""Independent arithmetic and identity audit of V12 confirmation, no fits."""
...
from bf_tap_r2.v12_confirm import four_seed_references
root=Path.cwd();out=root/'local/runs/round2-v12-joint-tabm/confirmation-r1'
spec=yaml.safe_load((root/'configs/round2_v12/SPEC.yaml').read_text());conf=yaml.safe_load((root/'configs/round2_v12/CONFIRMATION.yaml').read_text())
...
assert summary['selected']==conf['selected']=={'target':'tap_iron','recipe':'joint_plr001'}
assert file_hash(root/'configs/round2_v12/SPEC.yaml')==manifest['spec_sha256']
assert file_hash(root/'configs/round2_v12/CONFIRMATION.yaml')==manifest['confirmation_sha256']
...
for s,foldvec in folds.items():
 for f in range(5):
  ...
  if s in spec['split_seeds']:
   key=f'joint-joint_plr001-s{s}-f{f}';path=dev/(key+'.npy');assert file_hash(path)==dev_audit['hashes'][key]
  else:
  ...
  p=np.load(path,allow_pickle=False);assert p.shape==(int(mask.sum()),2) and np.isfinite(p).all();pred[s][mask]=p[:,0];checked[str(path.relative_to(root))]=file_hash(path)
assert all(np.isfinite(p).all() for p in pred.values())
y=frame.tap_iron.values
...
row=next(r for r in dev_summary['records'] if r['target']=='tap_iron' and r['recipe']=='joint_plr001')
