"""Zero-fit independent arithmetic and identity audit for V15 development."""
import argparse
import hashlib
...
from bf_tap_r2.v15_task_experts import load_references
...
def audit(root, out, destination):
    ...
    for seed in folds:
        np.testing.assert_array_equal(platform[seed]['tap_iron'],base[seed]['tap_iron'])
        expected=q20[seed]['tap_time_len']+(spec['platform_reference_time_alpha']-.2)/(.35-.2)*(base[seed]['tap_time_len']-q20[seed]['tap_time_len'])
        np.testing.assert_allclose(platform[seed]['tap_time_len'],expected,rtol=0,atol=1e-10)
    ...
    for name in spec['recipes']:
        ...
        for s,fv in folds.items():
            for f in range(5):
                ...
                gate=meta['gate_diagnostics']; learned=spec['recipes'][name]['learned_gates']
                assert gate['learned']==learned and np.isfinite(gate['parameter_norm']) and gate['parameter_norm']>=0
                for key in ['mean_weights','min_weights','max_weights']:
                    value=np.asarray(gate[key]);assert value.shape==(2,spec['expert_policy']['n_experts'])
                ...
                assert np.asarray(gate['mean_entropy']).shape==(2,)
                assert all(0<=x<=np.log(spec['expert_policy']['n_experts'])+1e-6 for x in gate['mean_entropy'])
                assert 0<=gate['task_mean_absolute_weight_difference']<=1
                assert meta['parameter_count']>=meta['trainable_parameter_count']>0
                if not learned:
                    assert gate['parameter_norm']==gate['task_mean_absolute_weight_difference']==0
                    for key in ['mean_weights','min_weights','max_weights']:check(gate[key],np.full((2,2),.5))
                else:
                    assert meta['parameter_count']==meta['trainable_parameter_count']
    ...
    for row in summary['records']:
        ...
        for label,ref in [('A60',platform),('A35',base),('Q20',q20),('CURRENT',current)]:
            ...
            for held,fv in folds.items():
                ...
                if label in ('A60','CURRENT') and g<=0: ok=False
                if label=='CURRENT':
...
if __name__=='__main__':
    ...
    assert args.run.resolve().is_relative_to(root/'local/runs/round2-v15-task-experts')
