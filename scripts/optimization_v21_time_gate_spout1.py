"""Build the complementary spout-1-only black-box challenger.

TGATE changed the low-support rows for both spouts.  TGATE_SPOUT2 changed
only spout 2 and produced the observed group-test score 83.1549.  This
package applies the exact TGATE rule only to spout 1, leaving every other
field byte-for-byte sourced from the frozen V10 result.  The package is
created separately and never overwrites V10 or an earlier challenger.
"""
...
def parse_time(frame: pd.DataFrame, column: str) -> None:
    ...
    frame[column] = (
        value.dt.tz_localize("Asia/Shanghai")
        if value.dt.tz is None
        else value.dt.tz_convert("Asia/Shanghai")
    )
...
def recent_spout_medians(test: pd.DataFrame, history: pd.DataFrame) -> np.ndarray:
    ...
    for row in test.itertuples(index=False):
        ...
        if local.empty:
            local = history.loc[
                (spout == str(row.spout_no)) & (history.tap_end_time <= CUTOFF)
            ]
        if local.empty:
...
def build(output: Path) -> dict:
    ...
    spout1 = test.spout_no.astype(str).to_numpy() == SPOUT
    gate = (effective < EFF_THRESHOLD) & spout1
    ...
    archive = submission / "Luqhhh_bf_tap_predict_prelim_TGATE_SPOUT1.zip"
    ...
    with ZipFile(archive) as handle:
        if handle.namelist() != ["result.csv"]:
            raise ValueError("challenger archive must contain only result.csv")
        if hashlib.sha256(handle.read("result.csv")).hexdigest() != sha256(csv_path):
            raise ValueError("ZIP payload differs from result.csv")

    summary = {
        "status": "PASS_READY_TO_SUBMIT_EXPLORATORY",
        "candidate": "T_GATE_V10_QRF_MEDIAN_RECENT60_SPOUT1_ONLY",
        "stage": "test_a",
        "rows": ROWS,
        ...
        "score_algebra": {
            "known_v10": 83.1951,
            "known_tgate": 83.1973,
            "known_tgate_spout2": 83.1549,
            "implied_spout1_only": 83.1951 + (83.1973 - 83.1549),
            "note": "linear score difference under unchanged iron and disjoint row masks",
        },
        "inputs": {
    ...
    (output / "challenger_manifest.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    candidate.assign(
        effective_neighbors=effective, gate=gate, recent60=recent
    ).to_csv(output / "prediction_audit.csv", index=False, float_format="%.6f", lineterminator="\n")
    return summary
