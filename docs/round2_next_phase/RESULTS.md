# Round2 下一阶段：批次 0 与批次 2 增量检验结果

日期：2026-09-24
分支：`codex/round2-next-phase-search`
代码：`src/bf_tap_r2/next_phase_probe.py`、`tests/test_round2_next_phase_probe.py`
计划：`docs/round2_next_phase/PLAN.md`

本文件记录**已实际执行**的结果与由此得出的负证据。运行产物在私有 `local/runs/round2-next-phase/`。

---

## 0. 复现校验（顺带完成）

用冻结配方 `configs/round2_v0_1/models.yaml` 的 **C2**（CatBoost RMSE / depth 6 / iterations 1500 / lr 0.03 / l2 10 / seed 42）在种子 42/3407 五折上重跑。

记录口径说明：`configs/round2_v2/comparison.yaml` 的 `selection` 是 `lowest_two_seed_mean_wmape`，所以历史登记的 C2 数值是**两种子均值**，不是单种子。

| | seed 42 | seed 3407 | **两种子均值** | 历史记录 C2 | 差 |
|---|---:|---:|---:|---:|---:|
| 铁量 WMAPE | 0.04006564 | 0.03977273 | **0.03991918** | 0.03991918 | 0 |
| 时长 WMAPE | 0.04101036 | 0.04049742 | **0.04075389** | 0.04075389 | 0 |

两种子均值的铁量/时长 WMAPE 与 V2 时代登记的 C2 **在 8 位小数上完全一致**；由两者合成的 J 为 0.0403365，与记录的 0.0403365373 也一致。说明本机的数据、折协议与配方能够精确复现历史基线——这是后续一切 delta 可比性的前提。

---

## 1. 批次 0：线性探针

问题：**每个目标的方差有多少是可被线性解释的？** 该答案决定交互特征批次是否有空间。

三种设计，全部只用当前折的训练部分拟合（缩放统计量也从训练部分冻结），预测跨折池化后用冻结的包分公式评分：

| 设计 | seed 42 | seed 3407 | 两种子 |
|---|---:|---:|---:|
| `raw`（21 特征 + 铁口 one-hot） | 91.7620 | 91.7730 | — |
| `four`（+4 个比值特征） | 92.4931 | 92.5048 | — |
| `degree2`（+210 个两两乘积） | 93.5791 | 93.6079 | — |

对照：全局中位数基线 ≈ 82.3；C2 ≈ 95.97；V34_A = 96.1997。

**结论**

1. **"目标方差大部分可被线性解释"这一假设被证伪。** 纯线性 `raw` 的铁量 WMAPE 为 7.45%，C2 为 3.99% —— 误差接近两倍，线性模型比当前最好低 **4.4 分**。先前从单变量相关系数（`oxygen` 0.73）推出的"强主效应已吃掉大部分信号"不成立。
2. **二阶交互确实带信号。** 加入全部两两乘积后 +1.82 分（铁量 WMAPE 相对降 26%，时长降 19%），两个种子、两个目标方向完全一致。
3. **但即使给足全部两两交互，线性模型（93.58）仍远低于 C2（95.97）** —— 存在大量二阶以上的结构。

---

## 2. 批次 2 增量检验：显式交互特征对树模型是否有增量

批次 0 证明的是"**线性模型**需要显式交互"，**不能**推出"**树模型**需要"。V3.4 的 EBM 自带 `interactions` 项，CatBoost 的树分裂本就能表达任意阶交互，因此必须直接测增量。

同一 C2 配方、同一折，只改输入特征：

| 设计 | 两种子均值 铁量 WMAPE | 两种子均值 时长 WMAPE | 包分 |
|---|---:|---:|---:|
| `raw` | 0.039919 | 0.040754 | **95.9663** |
| `degree2`（+210 个两两乘积） | 0.040690 | 0.042335 | **95.8488** |

**结论：显式两两交互特征对树模型是负增量，−0.1175 分，两个目标同时退化。**

树模型已经捕捉了这部分结构；额外 210 个特征只带来稀释。

### 由此产生的负证据（登记在案）

- **盲目铺开全局两两交互特征 → 关闭。** 不要再以"线性模型受益"为由推进该方向。
- 注意本检验**只否证了"全量两两展开"**。计划中允许的"由开发残差定向选出的**少数**交互"未被直接否证，但此结果显著提高了它的举证门槛：任何定向交互都必须先证明相对 `raw` C2 有增量。

---

## 3. 步骤 0a：嵌套外层驱动与 R0 参照

`V3.4` 的最终外层驱动从未进仓库（`constrained_forward_select` 在仓库内无调用方），新搜索线无法在记录所用的协议下评估候选。已补上：`src/bf_tap_r2/next_phase_nested.py`。

每个外层折内的契约：成员在**内层训练部分**拟合产生 inner OOF → 权重**只用**这份 inner OOF 学 → 成员在**完整训练部分**重训 → 才预测 valid。验证标签全程不读。测试用一个「只返回训练时见过的标签」的成员直接攻击该边界：正确的嵌套下它必须对每一行返回 0。

### R0 参照

`R0` = 冻结的 C2 配方（`configs/round2_v0_1/models.yaml`），**不是 V34_A**。历史成员池在本机不可恢复，R0 是自行重建的可复现锚点。

| outer seed | 分数 | 铁量 WMAPE | 时长 WMAPE |
|---|---:|---:|---:|
| 42 | 95.946200 | 0.04006564 | 0.04101036 |
| 3407 | 95.986493 | 0.03977273 | 0.04049742 |
| **16061** | **95.958389** | 0.04033063 | 0.04050158 |

**框架正确性验证**：R0 在 seed 42/3407 上的两个 WMAPE 与 §0 的普通 CV 结果**逐位相同**。单成员时嵌套 LP 必然给出权重 1.0，嵌套框架因此必须退化为普通折 CV —— 它确实做到了 bit-for-bit，说明 inner OOF → LP → 重训 → 预测 整条链无泄漏且实现正确。

### 参照强度对照（同协议，不同成员池）

| | outer 16061 分数 |
|---|---:|
| R0（仅 C2） | 95.958389 |
| V34_A（L1 + EBM 专家） | 96.199737 |
| 差 | **−0.241348** |

**R0 是弱锚点，这是本机没有历史缓存的直接代价。** 距本地门槛 96.25 名义上差 0.29，而 V34_A 只差 0.05。因此：

- R0 上测出的 **delta（相对增量）** 用于机制筛选是有效的
- 但 **R0 上的绝对分不能用于晋级声明**，也不能与 96.25 直接比较
- 弱锚点还有一个风险：某个专家在 R0 上有增量，**不代表**它在 V34_A 更强的基座上仍有增量（可能与 V34_A 已有的 EBM 专家冗余）。任何"超出 V34_A"的声明都需要 0b

---

## 4. 批次 1：时长侧 EBM 专家（开发切分）

在 R0 上加入两个时长侧 EBM 边界专家（冻结网格的两个点），走完整嵌套外层协议。

| outer seed | 铁量 WMAPE | 时长 WMAPE | 分数 | 时长相对改善 |
|---|---:|---:|---:|---:|
| R0（42） | 0.04006564 | 0.04101036 | 95.946200 | — |
| **R0 + 时长 EBM（42）** | 0.04006564 | **0.03950980** | **96.021228** | **−3.66%** |
| R0（3407） | 0.03977273 | 0.04049742 | 95.986493 | — |
| **R0 + 时长 EBM（3407）** | 0.03977273 | **0.03936810** | **96.042959** | **−2.79%** |

铁量逐位不变，因为 EBM 成员声明只服务时长目标，铁量侧仍只有 C2 且权重为 1.0。

LP 权重（fold 0）：

```
seed 42  : c2_raw 0.3420 | b128_l30_i20 0.2124 | b256_l60_i40 0.4456
seed 3407: c2_raw 0.4077 | b128_l30_i20 0.3036 | b256_l60_i40 0.2886
```

**结论**

1. **两个完整开发切分同时为正**，达到计划的晋级条件（+0.0750 / +0.0565 分）。
2. 两个 EBM 点都拿到实质权重，说明它们与 C2 互补，不是噪声。
3. **但这条结果主要是确认，不是新发现。** "EBM 专家在基座上做增量"正是 V3.4 已经建立的机制（V34_A 相对 L1 的 +0.0394 就来自 EBM 专家）。本批次在自建锚点上复现了该机制，验证了框架，但未提供超出 V3.4 的新方向。
4. 与 V34_A 的对照只能定性：V34_A 的开发期时长 WMAPE 为 0.038423567，仍优于本批次最好值 0.03936810。差距来自基座（L1 成员池 vs 单一 C2），不是专家本身。

### 尚未回答的问题

**时长侧残差专家仍未验证。** V3.4 因 V3.3 只记录了一个时长 EBM 中心，其 16 个时长残差槽位有 8 个被迫标 `not_applicable`，且最终没有任何残差专家被选入 A。这才是批次 1 里信息量最大的部分，本批次尚未触及。

### 关于 18041

计划规定"两个完整开发切分为正才允许消耗新的最终外层种子"，本批次满足该条件。但**不建议现在动用 18041**：该条件是为"可晋级的候选"设的，而 R0 是弱锚点、其上产物无法与 V34_A 比较，消耗外层种子得不偿失。18041 应留给 0b 之后、在可比基座上产生的候选。

---

## 5. 批次 1：分铁口收缩专家（开发切分）

在 R0 上加入两个分铁口收缩候选（`local_l2_multiplier ∈ {1.0, 3.0}` × `beta ∈ {0.25, 0.50}`），走完整嵌套外层协议。

| outer seed | R0 | R0 + 收缩 | 收缩权重 |
|---|---:|---:|---|
| 42 | 95.946200 | **95.946200** | 0.0000 / 0.0000 |
| 3407 | 95.986493 | **95.986493** | 0.0000 / 0.0000 |

**LP 在两个开发切分上都给两个收缩成员恰好 0 权重，分数与 R0 逐位相同。**

机制解释：训练集只有 2 个铁口、每口约 1370 行，局部模型与全局模型的结构几乎一致，`global + β·(local − global)` 的修正项携带的独立信号极少。这与 `spout_no` 的弱边际相关（铁量 −0.023 / 时长 +0.183）一致，也与 `min_spout_samples = 200` 在该样本量下不构成约束这一事实一致。

### 负证据的边界（重要）

本结论**不能**推广为"分铁口路线整体关闭"：

- 它证明的是「**C2 父配方** + 分铁口修正」在时长上无效
- V34_A 的收缩专家 `global_spout_shrink-0136` 用的是**另一个父配方**（`v31-s1-time-0021-0050`），且确实在 A 中被选中（时长权重 0.094）
- 父配方的差异只能由 0b 回答；本筛选的父配方是替身（见 `next_phase_members.SHRINK_PARENT_PARAMS` 注释）

另外，A/B 两个平台分仅差 0.0024，而它们唯一的实质差异就是"用收缩专家还是用残差专家"，这也说明该方向的效应量在噪声量级。

---

## 6. R1 强锚点（开发切分）

R0 是单一 C2 成员，候选相对它显得有用可能只是因为基座弱——批次 1 的三个筛选有两个因此无法与 V34_A 比较。R1 刻意**包含**批次 1 确认有效的时长 EBM 专家，使后续筛选提出的问题变成 V34_A 真实面对的那个：在已经带有已知有效机制的基座上，还有增量吗？

成员池（全部可从公开 config 复现）：`c2_raw`（冻结 C2）、`d4_shallow`（V2.1 的 D4：depth 4 / 3000 iters）、`ord_ordered`（V2.5 的 ORD：Ordered boosting）、两个时长 EBM 边界专家。两个 CatBoost 变体是**结构性**差异（容量、boosting 类型），不是换种子。

| pool | seed 42 | seed 3407 |
|---|---:|---:|
| R0 | 95.946200 | 95.986493 |
| R0 + 时长 EBM | 96.021228 | 96.042959 |
| **R1** | **96.063181** | **96.081358** |

逐目标 WMAPE：

| pool | 铁量 42 | 时长 42 | 铁量 3407 | 时长 3407 |
|---|---:|---:|---:|---:|
| R0 | 0.04006564 | 0.04101036 | 0.03977273 | 0.04049742 |
| R1 | 0.03948823 | 0.03924815 | 0.03926056 | 0.03911227 |

LP 权重（fold 0）：

```
铁量  42    : c2_raw 0.1490 | d4_shallow 0.6411 | ord_ordered 0.2099
铁量  3407  : c2_raw 0.0975 | d4_shallow 0.6205 | ord_ordered 0.2820
时长  42    : c2_raw 0.0000 | d4_shallow 0.0995 | ord_ordered 0.3205 | ebm1 0.2289 | ebm2 0.3511
时长  3407  : c2_raw 0.0000 | d4_shallow 0.2053 | ord_ordered 0.2854 | ebm1 0.2749 | ebm2 0.2343
```

**结论**

1. **C2 在时长上拿到 0 权重。** 一旦成员池有了多样性，作为 R0 唯一成员的 C2 就不再被使用。这从数据上确认了 R0 是个差基座，也说明批次 1 前两个筛选是在接近无用的基座上做的。
2. **`d4_shallow` 在铁量上占约 0.64 权重**，压过 C2（depth 6 / 1500）。容量与训练轮数的配比比"更深更好"更重要。
3. **两个时长 EBM 在更强基座上合计仍占 0.51–0.58 权重**，说明批次 1 的结论不是 R0 太弱造成的假象。
4. **R1 仍低于 V34_A 的开发期水平（约 96.19）**，大致补上了 R0→V34_A 差距的一半。

**边界**：R1 不是身份可比的 V34_A 参照，成员池由本计划自行定义。它让筛选**可解释**，但"达到 96.25"这类晋级声明仍需 0b。

---

## 7. R1 扩展：结构多样性到达平台期

在 R1 上加入三个结构性 CatBoost 变体（容量 `d8_deep`、正则 `l2_strong`、特征子采样 `rsm_half`）以及一个多目标联合成员 `joint_multirmse`。

| pool | seed 42 | seed 3407 |
|---|---:|---:|
| R1 | 96.063181 | 96.081358 |
| **R1 扩展** | **96.064394** | **96.086693** |
| delta | +0.0012 | +0.0053 |

LP 权重（fold 0）：

```
d8_deep         : 铁量 0.000 / 0.000   时长 0.000 / 0.000
rsm_half        : 铁量 0.000 / 0.000   时长 0.000 / 0.000
l2_strong       : 铁量 0.000 / 0.000   时长 0.059 / 0.006
joint_multirmse : 铁量 0.498 / 0.262   时长 0.000 / 0.000
```

**结论**

1. **结构多样性作为杠杆已经榨干。** 两个变体在全部四种组合下拿到 0 权重，第三个只在时长上有 0.006–0.059 的边际权重，整体增益 +0.0012 / +0.0053。继续增加同类 CatBoost 变体没有意义。
2. **`joint_multirmse` 在铁量上拿 0.26–0.50、在时长上拿 0，两个种子完全一致。** 这**在不同基座上独立复现了 V2.3 的记录**（"MultiRMSE 只帮铁量，对时长无帮助"）。原记录来自单次实验；本次是在自行重建的成员池与不同折种子上的复现，因此更有说服力。
3. 由此，计划中"非对称跨目标输入"的方向获得了实证支持，**且应指向铁量侧**：跨目标信息确实存在，但只在铁量上可利用。注意该收益已被 R1 通过 `joint_multirmse` 捕获，新设计必须证明**超出**它的增量。

---

## 8. 路径 D：L0 时长侧重建（seed 42）

§3 的调查结论是 L0 七个成员里六个可重建、`AJM1` 不可。本节的模块重建**时长侧**——三个成员全部可追溯，不涉及任何猜测：

| 成员 | L0 时长权重 | 配方来源 |
|---|---:|---|
| `v3-catboost-tap_time_len-0021` | 0.668858 | 冻结 V3 sampler 日程（种子化随机，种子固定故可重放） |
| `AORD` | 0.174110 | `configs/round2_v2_5`：B3 与 ORD 的均值 |
| `T1_B3_PREFIX1000` | 0.157032 | 同三成员截断到前 1000 棵树 |

B3 = C2 在种子 42/2026/2027 的均值（`v2_refinement.new_model` 的 `int(member.split("_")[-1])` 证实种子绑定）。

### 三组对照（时长，嵌套协议，同折）

| pool | seed 42 WMAPE | seed 3407 WMAPE | ΔS（折算包分） |
|---|---:|---:|---:|
| R1 扩展版（9 成员，折内调权） | 0.03922786 | 0.03911320 | — |
| L0 冻结权重（3 成员，单成员评估） | 0.03915999 | 0.03945085 | +0.0034 / **−0.0169** |
| R1 扩展 + 3 个 L0 成员（12 成员） | 0.03882376 | 0.03896767 | +0.0202 / +0.0073 |
| **再加 L1 的 4 个 v31 成员（16 成员）** | **0.03851698** | **0.03846710** | **+0.0355 / +0.0323** |

### 追加对照：加入真 L1 成员后自建成员全部归零

第 4 行加入的是 L1 时长池实际使用的四个 v31 变异成员。权重（fold 0）：

```
seed 42 : v31_time_0039 0.4413 | v31_time_0050 0.2057 | ebm_b128 0.1283
          ebm_b256 0.1188 | v3_0021 0.1060
seed 3407: v31_time_0039 0.3220 | ebm_b128 0.2398 | v31_time_0050 0.2388
          v31_time_0031 0.1208 | v3_0021 0.0786

c2_raw = 0 | d4_shallow = 0 | d8_deep = 0 | l2_strong = 0 | ord_ordered = 0
joint_multirmse = 0 | rsm_half = 0 | aord = 0 | t1_b3_prefix1000 = 0
两个种子上，全部自建成员权重为 0
```

**这是本节最重要的发现，也是对 §7 的最终修正：**

1. 一旦真正的 L1 成员可用，**本计划自建的 12 个成员全部获得 0 权重**，两个开发切分一致。
2. 因此 §8 前三行的"增益"（+0.0202 / +0.0073 / +0.0034）**不是新能力，而是补上了本计划自己缺失的成员**。这些成员 V34_A 的 L1 池里本来就有。
3. 本计划对时长侧的**全部实际贡献只剩两个 EBM 边界专家**（`time_ebm_b128` / `b256`，合计权重 0.247 / 0.240），而 V34_A 的专家池里本来就有同类 EBM 专家（`ebm_boundary-0089`）。
4. **结论：在 §7–§8 的全部工作中，没有任何一项超越 V34_A。** 相关分数不得作为"提升"引用。

权重（fold 0，两种子并列）：

```
v3-catboost-tap_time_len-0021 = 0.4541 / 0.5582   ← 两种子均占最大权重
time_ebm_b128_l30_i20         = 0.2126 / 0.2309
time_ebm_b256_l60_i40         = 0.1833 / 0.0777
ord_ordered                   = 0.1031 / 0.0621
l2_strong                     = 0.0469 / 0.0590
t1_b3_prefix1000              = 0.0000 / 0.0122   ← 仅 3407 有微量
aord = 0.0000 / 0.0000 | c2_raw = 0 | d4_shallow = 0 | d8_deep = 0
joint_multirmse = 0 | rsm_half = 0
```

### 结论

1. **§7 的"成员多样性到达平台期"结论需要修正，但修正后的读法比原先更强。** 平台期是**本计划自选成员的**平台期。加入历史成员得到 +0.0202 / +0.0073 / +0.0323（三批累计 +0.0355 / +0.0323），而自建新变体只得 +0.0012 / +0.0053。
2. **但这些"增益"不是新能力，而是补缺口。** 第 4 行加入的 v31 成员与第 3 行的 L0 成员，**V34_A 的 L1 池里本来就有**。加入它们之后自建成员全部归零（见上），说明我们做的只是把池子恢复到 V34_A 已经达到的水平。
3. **本计划对时长侧的全部实际贡献只剩两个 EBM 边界专家**，而 V34_A 的专家池里本就有同类专家（`ebm_boundary-0089` 在 A 中占时长权重 0.287）。
4. **L0 的冻结权重并不稳定优于重新调权**——它在 seed 42 上小胜（+0.0034）、在 seed 3407 上明显落后（−0.0169）。原因是它把 0.331 的权重压在 `AORD` 与 `T1_B3_PREFIX1000` 上，而二者与 V3 trial 同源、高度相关。**单看 seed 42 会得出错误结论**，这是必须两种子都看的实例。
5. 补全后的等效总分（铁量取 R1 扩展版、时长取本行）：

| | seed 42 | seed 3407 |
|---|---:|---:|
| 等效总分 | 约 96.0997 | 约 96.1136 |
| 两种子均值 | **96.1067** | |
| 距本地门槛 96.25 | **−0.1433** | |

### 边界与使用限制

- 本节**只建立时长侧可比性**，不是 V34_A 的全模型身份。
- 单成员评估（L0 冻结权重）的内层 OOF 步骤是平凡的（权重必为 1.0），与多成员池比较时存在这一协议差异。
- **§8 全节的分数不得作为"相对 V34_A 的提升"引用。** 它们是自建池在不同完整度下的自我对照，补全后收敛到 V34_A 已有的水平，而非超越。
- 两个完整开发切分同向为正，但**没有**由此消耗新的最终外层种子。

### 在 outer 16061 上与 V34_A 的直接比较（排除"只换时长列"路线）

把补全后的时长池放到 outer 16061、内层改用真实驱动的 3 折与 `inner_seed=7771+fold`：

| | 时长 WMAPE |
|---|---:|
| **本计划补全后的时长池** | **0.03818417** |
| V34_A（原件） | 0.038149803 |
| 差 | **+3.4e-5（相对 +0.09%）** |

权重（fold 0）：`v31_time_0039` 0.2989 · `v31_time_0050` 0.3835 · `ebm_b128` 0.1420 · `ebm_b256` 0.1044 · `v31_time_0031` 0.0711；**其余 11 个自建成员全部为 0**。

**结论：本计划的时长模型与 V34_A 的时长侧基本打平、微弱落后。** 因此"保留 V34_A 铁量列、只替换时长列"的受控平台替换**没有价值**——换上去大概率只是把同一个结果再交一遍。该路线从候选梯队中划掉。

---

## 9. V3.1 日程重放（并已验证）

§3 的结论是 `AJM1` 不可重建、L1 因而不可恢复。该结论对**铁量侧的一个成员**仍然成立，但**对 L1 整体不成立**。

### 为什么可以重放

- `v3_1_sampler.load_centers` 读取 `local/runs/round2-v3-local-search/coarse-catboost-r1/fit_ledger.jsonl`，该文件缺失，故 V3.1 日程看似不可恢复
- 但 `v3_run.trial_details` 显示该 ledger **只承载 trial 定义**，此外无他
- 而六个父配方（`TIME_CENTERS` 3 个 + `IRON_CENTERS` 3 个）**全部是 V3 第一批 trial**，由 `v3_local_search.sample_trials` 从冻结搜索空间确定性重放

因此父配方是**查出来的**，不是编出来的。`v3_1_sampler.sample_s1` 增加了可注入的 `centers` 参数，默认路径不变（有测试钉住：省略参数仍走 ledger 并抛出原错误）。**未向 `local/runs` 写入任何内容，未伪造运行记录。**

### 验证（关键）

重放成立的前提是：当初 V3 粗筛使用了默认采样种子 `20260923`。该假设可用 V3.1 记录中的具体分数检验。

**首次验证失败**：误用了 `evaluate_trial_folds`（90/10 随机内切分），两个 trial 系统性偏高于记录值 +0.0004~+0.0005。

**改用正确协议后通过**——V3.1 用的是 `evaluate_v31_outer_folds`（内层 `make_folds`），且 `inner_seed = 9000 + i`（`i` 为该 trial 在日程中的位置）：

| trial | 位置 i | 重放 pooled WMAPE | 记录值 | 偏差 |
|---|---:|---:|---:|---:|
| `v31-s1-expr-iron-0018` | 234 | 0.03898311 | 0.03900850 | **−0.0000254** |
| `v31-s1-time-0021-0050` | 50 | 0.03897805 | 0.03898744 | **−0.0000094** |

偏差 9.4e-6 ~ 2.5e-5（相对 0.024% ~ 0.065%）。

### 事后证实：与真实 ledger 逐参数一致

上述分数检验之后，真实运行产物已通过单独渠道取回（`local/handoff-20260924/`，第 2.2/2.3 档）。直接比对：

| 比对项 | 结果 |
|---|---|
| 六个 centers vs `round2-v3-local-search/coarse-catboost-r1/fit_ledger.jsonl` | **6/6 IDENTICAL** |
| S1 全部 320 项 vs `round2-v3.1-directed-search/s1-coarse-r1/fit_ledger.jsonl` | **320/320 IDENTICAL，零差异** |

比对使用 `json.dumps(sort_keys=True)` 的规范化字典比较，覆盖嵌套的 `parameters`。这不是"分数接近"，而是**逐参数完全相同**。

因此早先那句"无法验证前提假设本身"不再成立：**前提假设已被证实**。分数检验（9.4e-6）与参数检验（320/320）互相独立，且指向同一结论。

### 对 0b 的影响

L1 的成员池现在大部分可重建：

| 侧 | 成员 | 状态 |
|---|---|---|
| 时长 | `v31-s1-time-0021-0050` / `-0031` / `-0039` / `-0002` | ✅ 可重放 |
| 时长 | V3 `tap_time_len-0021` | ✅ 可重放（§8 已用） |
| 时长 | L0 时长侧 | ✅ 可重建（§8） |
| 铁量 | `v31-s1-expr-iron-0018` / `-0012` | ✅ 可重放 |
| 铁量 | `J1` | ✅ 配方在 `configs/round2_v2_3` |
| 铁量 | L0 铁量侧 | ⚠️ 六/七可重建，缺 `AJM1` |
| 铁量 | **`AJM1`** | ❌ **仓库内无定义，仅见于 docs** |

**即：铁量侧只差 `AJM1` 一个成员。** 若该定义可得（见 `local/artifact-request-2026-09-24.md` 第 3 档），0b 即可完整进行。

---

## 10. 复现 V34_A：原件驱动在本机跑通

拿到完整交接材料后（含真实运行目录、`run_v34_final_outer.py`、`run_l1_oof.py`），把 `local/runs/round2-*` 还原到位，仅改动脚本中硬编码的 `ROOT`，并把输出指向 `final-outer-16061-repro-r1`（**不覆盖原件目录**）。

### 结果

| 指标 | 原件 | 复现 | 差 |
|---|---:|---:|---:|
| candidate 包分 | 96.19973698527157 | **96.19837140814283** | **−0.00137** |
| L1 包分 | 96.16031215578445 | 96.15686584533933 | −0.00345 |
| candidate 铁量 WMAPE | 0.037855457487617636 | 0.037882199754914254 | +2.7e-5 |
| candidate 时长 WMAPE | 0.03814980280695106 | 0.03815037208222921 | +5.7e-7 |
| L1 铁量 WMAPE | 0.038511981119337194 | 0.0385809073282394 | +6.9e-5 |
| L1 时长 WMAPE | 0.038281775764973866 | 0.038281775764973956 | **+9e-17** |

### 逐样本比对（2754 行，折号全部一致）

| 列 | max\|差\| | 平均\|差\| | 相对 | >0.01 的样本 |
|---|---:|---:|---:|---:|
| `l1_tap_time_len` | **0.000000** | 0.000000 | **2.2e-15** | **0** |
| `candidate_tap_time_len` | 0.154 | 0.0057 | 4.5e-5 | 424 |
| `candidate_tap_iron` | 1.348 | 0.185 | 3.4e-4 | 2647 |
| `l1_tap_iron` | 2.572 | 0.368 | 6.9e-4 | 2703 |

### 判读

1. **时长侧基础成员逐位复现**（`l1_tap_time_len` 的 max 差为 0，相对 2.2e-15）。这证明折协议、成员配方、LP 权重学习全部正确——**协议本身无差**。
2. **差异集中在铁量侧**（相对 7e-4）**与时长侧的 EBM 专家**（相对 4.5e-5）。铁量侧独有的成分是 `j1_*`（MultiRMSE 联合）与 `jm1`（**MLP，lbfgs 求解**）。
3. JM1 在本环境内**两次运行逐位相同**，故非进程内随机性。跨环境差异的首要嫌疑是 scipy/sklearn 的 `lbfgs` 实现差异——`nonlinear_models` 的 MLP 用 `lbfgs` + `tol=1e-6` + `max_iter=4000`，此类优化器对库版本敏感，末位差异会在迭代中放大。运行目录中**没有环境记录**可供比对，故该归因是首要假设而非已证实结论。
4. 净影响：包分差 **0.0014**（0.0014%）。对方向判断无实质影响。

### 使用口径

- **作对照时使用原件 `final-outer-16061/final_oof.csv`**，那是平台最佳那次运行的逐样本预测
- **本机复现件用于验证管线可跑通，以及作为后续候选的评估 harness**（改 `BASE_NAMES` 即可接入新成员）
- 不得把复现件的分数当作原件分数引用

---

## 11. 组合约束放宽（负证据）

V34_A 的铁量侧在 5 折中有 4 折 `new_weight_total` **恰好顶在 0.50**，是计划点名要检查的"权重是否长期贴边"情形。用原件驱动在同一趟里扫描多个上限（成员预测与约束无关，故一次运行可评估全部）：

| `max_new_weight` | 包分 | 铁量 WMAPE | 时长 WMAPE |
|---|---:|---:|---:|
| 0.50（V34_A 原设置） | 96.198371 | 0.037882200 | 0.038150372 |
| 0.65 | 96.199422 | 0.037861190 | 0.038150372 |
| 0.80 | 96.199422 | 0.037861190 | 0.038150372 |
| **1.00（无约束）** | **96.199422** | 0.037861189 | 0.038150372 |
| L1 对照 | 96.156866 | 0.038580907 | 0.038281776 |

**结论：约束确实在起作用，但放宽它几乎没有收益（+0.00105）。**

1. 从 0.50 放宽到 1.00 只带来 **+0.00105** 分（铁量 WMAPE 改善 5.5e-5）。
2. **cap ≥ 0.65 之后结果完全相同**，说明无约束最优解的新专家权重本身就落在 0.65 以内——0.50 的约束把它压住了，但被压掉的部分价值极小。
3. 由此产生一条明确的负证据：**"组合权重上限是主要瓶颈"这一假设不成立**，不要再以放宽约束作为方向。
4. 附带观察：本机 cap=0.50 的复现值 96.198371 与原件 96.199737 之间，恰好夹着 cap=0.65 的复现值 96.199422。这与 §10 的跨环境数值漂移一致，也说明原件那个解在本机精度下无法逐位重建。

输出：`local/runs/round2-v3.4-ebm-and-constrained-composition/final-outer-16061-sweep-r1/sweep_summary.json`。

---

## 12. 平台回传：三候选因子设计（用户报告，未独立核验）

2026-09-24 晚，用户说明当日有三个提交名额。据此提交了三个**在同一环境、同一外层折（16061）、同一内层协议**下评估的候选，覆盖两个此前从未测过的约束轴：

| 编号 | 包 | 改动的轴 | 本地（我方环境） |
|---|---|---|---:|
| — | V34_A（基线） | 无（ne2 · cap0.50） | 96.198371（本机复现） |
| **1** | `01_V34_NE3_CAP50` | **仅专家数 2 → 3** | **96.199574** |
| 2 | `01_V34_NE2_CAP65` | **仅权重上限 0.50 → 0.65** | 96.200941 |
| 3 | `01_V34_NE3_CAP65` | 两者同时 | 96.201458 |

三个包的设计意图与结构差异：

- **包 1（已回传）**：新专家数上限由 2 放宽到 3，权重上限保持 V34_A 的 0.50。铁量侧因此选入 **`v34-s1-ebm_residual-0100`——V3.4 从未成功选入过任何残差专家**；时长侧选入此前未使用的边界中心 **`ebm_boundary-0081`**。与 V34_A 原包逐行比对：**322/322 行全部不同**。
- **包 2**：专家数保持 2，权重上限放宽到 0.65。**时长列与 V34_A 逐位相同**（时长侧新专家权重仅 0.3808，上限不构成约束），差异只在铁量侧权重重新分配。
- **包 3**：两者同时放宽，铁量侧选入残差专家 `ebm_residual-0108`，时长侧与包 1 逐位相同。

设计目的：以已知的 96.2684 为基线，用三个格子定位"平台是否对某一轴敏感"，而不是提交同一方向的三个变体。

### 回传结果

**包 1 `01_V34_NE3_CAP50` = 96.2698**（用户报告，未独立账号核验）。

### 关键读数：本地 delta 与平台 delta 一致

| | 本地（我方环境，outer 16061） | 平台（用户报告） |
|---|---:|---:|
| V34_A | 96.198371 | 96.2684 |
| **包 1** | **96.199574** | **96.2698** |
| **delta** | **+0.001203** | **+0.0014** |

**同环境内的本地 delta（+0.0012）与平台 delta（+0.0014）相差 0.0002，几乎精确转移。**

这是本项目首次得到"本地 delta 可预测平台 delta"的证据。此前的证据只支持"本地分与平台分之间存在一个不稳定的加性差（+0.0687 ~ +0.1135）"；而**差值本身**（同一环境下两个候选之差）看来会转移。

**边界（必须一并记录）**：

1. 两个 delta 都只有 **0.001 量级**，与跨环境复现漂移（0.0014）同阶，也可能是一次巧合。
2. 平台分与本地分**排序不一致**：包 1 的本地分（96.199574）低于包 2、包 3，但它是目前唯一回传的。若包 2/包 3 回传后排名不同，则"本地排序可预测"仍然不成立——**可预测的只是 delta 的数值，不是排序**。
3. 单次回传，n=1。

**`96.2698` 目前是已知平台最好成绩**（原 96.2684），领先 0.0014。

> **本分支同步标注（2026-09-25 合入时补记，不改动上文历史记录）**
>
> 上句是 2026-09-24 时点的结论。本分支此后已收到用户回传的 **V36 = 96.2734**（`round2_current_candidate_queue`，同样为用户报告、未独立账号核验），当前已知平台最好成绩已由它持有；`96.2698` 作为该时点的历史证据保留，不再作为当前基准。两者均为用户回传，未经独立账号核验。

---

## 13. 对计划的影响

| 批次 | 原计划 | 调整后 |
|---|---|---|
| 批次 1（时长侧 EBM 残差） | 待定 | **提升为唯一主线**。线性地板 91.76 与当前最好 96.20 之间的 4.4 分全部来自非线性建模，残差/EBM 专家有真实空间 |
| 批次 2（交互特征） | 与分铁口收缩同批检验 | **交互特征一侧降级**（全量展开已关闭）；分铁口收缩一侧保留，仍需按原标准举证 |
| 批次 3、4 | 不变 | 不变 |

---

## 14. 复现方式

```bash
uv run --extra round2 python -m bf_tap_r2.next_phase_probe --mode linear   --root . --seeds 42 3407
uv run --extra round2 python -m bf_tap_r2.next_phase_probe --mode catboost --root . --seeds 42 3407
uv run --extra round2 python -m bf_tap_r2.next_phase_nested --pool r0         --root . --outer-seed 16061 --inner-seed 7771
uv run --extra round2 python -m bf_tap_r2.next_phase_nested --pool r0+time-ebm --root . --outer-seed 42   --inner-seed 7771
uv run --extra round2 python -m bf_tap_r2.next_phase_nested --pool r0+shrink  --root . --outer-seed 42    --inner-seed 7771
uv run --extra round2 python -m bf_tap_r2.next_phase_nested --pool r1         --root . --outer-seed 42    --inner-seed 7771
uv run --extra round2 python -m bf_tap_r2.next_phase_nested --pool r1-extended --root . --outer-seed 42   --inner-seed 7771
uv run --extra round2 python -m bf_tap_r2.next_phase_nested --pool l0-time              --root . --outer-seed 42 --inner-seed 7771 --targets tap_time_len
uv run --extra round2 python -m bf_tap_r2.next_phase_nested --pool r1-extended+l0time   --root . --outer-seed 42 --inner-seed 7771 --targets tap_time_len
uv run --extra round2 python -m bf_tap_r2.next_phase_nested --pool r1-extended+l0time+l1time --root . --outer-seed 42 --inner-seed 7771 --targets tap_time_len

# §10 - reproduce V34_A with the original driver.  Needs local/runs/round2-*
# restored from the handoff archive; the output directory is renamed so the
# original final-outer-16061 record is never overwritten.
uv run --extra round2 python local/runs/round2-v3.4-ebm-and-constrained-composition/run_v34_final_outer.py
```

线性探针约 4 秒；CatBoost 增量检验约 5 分钟（40 次拟合）；单个种子的 R0 嵌套外层约 3 分钟（60 次拟合）；单个种子的 `r0+time-ebm` 约 20 分钟；单个种子的 `r0+shrink` 约 15 分钟。

私有产物路径：`local/runs/round2-next-phase/linear-probe-r1/linear-probe.json`、`local/runs/round2-next-phase/catboost-increment-r1/catboost-increment.json`、`local/runs/round2-next-phase/r0-outer-{42,3407,16061}/r0-outer.json`。
