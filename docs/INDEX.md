...
## 当前 Round2 V4.3 真实强基座时长残差（2026-09-25）

- [V4.3 计划](round2_v4_3/PLAN.md)、[V4.3 结果](round2_v4_3/RESULTS.md)
- 把 V4.1 的公开弱锚点时长残差校准搬到**真实冻结 V36 组合**上重做，逐行对齐父包，私有 V3.4/V3.6 缓存齐备且父包开发分逐位重建为 **96.20376256899247**（与记录一致）。
- 预登记：seeds {42,3407} × folds {0,1} 粗筛；继续门槛为两 seed 同为正且平均整包增量 ≥ 0.005；未过门槛则不进完整五折。
- 结果：**无候选通过门槛**，最佳 `SB10_M50_A050` 平均 **+0.00179**（seed 42 为负）；距本地工作门槛 96.25 所需 +0.04624 差 0.04445。
- 关键负证据：V4.1 的最优候选 `HGB_L2_L15_A050` 在公开锚点上为 **+0.02571**，在真实 V36 父包上为 **-0.01450** —— 该增益来自修正公开重建锚点自身的失校准，**不可迁移**到真实强基座。
- 未生成平台包、未上传；当前平台最佳仍为用户回传 V36 = 96.2734，下一阶段目标不变（严格 > 96.3）。
