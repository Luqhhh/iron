...
截至 2026-09-15，最新完成的工程阶段为 platform-probes-r2（G0 通过、V11 关闭）；最近模型质量阶段仍为 v0.15（`FAIL_CLOSE_V8_RETAIN_V1`）。活动发布模型仍为 V1（用户回传 83.0319），初赛最高用户回传包为 V10（83.1951）；两者身份不得合并。
...
- [V11 均值探针反馈](../local/runs/platform-probes-r2-v11-feedback-r1/platform_feedback.json)：83.1806、比 V10 低 0.0145，决定 `CLOSE_V11_RETAIN_V10`；无新增拟合且不改写历史 G1。
...
| 阶段 | 冻结结果 | 阅读口径 |
| --- | --- | --- |
| platform-probes-r2 | [V10/V11 零拟合探针](platform_probes_r2/PLAN.md) | V10 独立复现、V11 均值时长封包；根382/worker32项测试；V11用户回传83.1806，关闭V11并保留V10最高回传；最近模型质量G1仍取v0.15 |
