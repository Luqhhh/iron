...
状态：**FROZEN_FOR_DISCUSSION**，随后获得用户回传平台分 **96.2684**，成为当前新的已知平台最佳。平台分来源为用户回传，未独立账号核验。
...
状态：**DEVELOPMENT_ALTERNATIVE_NOT_PROMOTED_BUT_PLATFORM_TESTED**。开发上略优于 A；随后作为第二顺序包用户回传平台分 **96.2660**，同样高于旧 AJ3 的 96.1259。B 仍没有独立最终外层确认，平台结果来自用户回传。
...
- A 已生成并写桌面，用户回传平台分 96.2684，当前新的已知平台最佳。
- B 已生成并写桌面，用户回传平台分 96.2660；保留为开发替代和平台已验证的第二结果。
- 条件追加扩展全部淘汰。
