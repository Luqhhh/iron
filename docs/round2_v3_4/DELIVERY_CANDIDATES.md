...
| 排序 | 包 | 状态 | 推荐理由 |
| ---: | --- | --- | --- |
| 1 | `01_V34_A_MECHANICAL_CONSTRAINED` | 最终外层 16061 已验证；用户回传 96.2684 | 同协议 `delta_L1 +0.0394248`，五折均为正；本轮唯一完成独立外层确认的 V3.4 流程 |
| 2 | `02_V34_B_DIVERSITY_CONSTRAINED_DEV` | 仅开发 seeds 42/3407 验证；用户回传 96.2660 | 多样性完整候选，开发 `delta_L1 +0.0438448`，两个开发 seed 均为正；与 A 的实质差异在时长专家，作为不同结构备用 |
...
- 本轮提交前平台已知最佳为 AJ3 铁量＋完整 B3 时长，用户回传 `96.1259`；它是提交前参照。此后 A 96.2684 和 B 96.2660 均高于该值，A 成为新的已知平台最佳。
...
| 包 | 平台分 |
| --- | ---: |
| A `01_V34_A_MECHANICAL_CONSTRAINED` | **96.2684** |
| B `02_V34_B_DIVERSITY_CONSTRAINED_DEV` | **96.2660** |

结论：

- A 为本轮用户回传第一名，也是当前新的已知平台最佳；B 紧随其后。
- 两者都高于此前 AJ3 的 `96.1259`。
- A 的 `delta` 相对此前平台最佳约 `+0.1425`；这只是回传分差，不是后续平台的保证。
- 本地回执：`local/runs/round2-v3.4-ebm-and-constrained-composition/release-candidates-r1/platform-feedback-r1/feedback.json`
- 平台回传来源为用户报告，不是独立账号核验；助手未上传。
