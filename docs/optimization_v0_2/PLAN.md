...
protected labels. The user reported `82.9543`, improving E09 by `0.1369`; this
external result changes the incumbent but does not alter the pre-existing G1 decision.

## OPT-06 pre-registration

OPT-06 tests target-level composition using only development evidence already frozen
by OPT-05. E12 has the stronger iron target mean; E14 has the stronger time target
mean and H1. E15 therefore takes iron exactly from E12 and time exactly from E14.
E16 applies the same previously frozen `1.68610975` time residual to E12 while
leaving its iron prediction unchanged. Both are deterministic nested derivations;
there is no weight or calibration grid, no model retraining, and the `82.9543`
platform result selects no parameter. The full run includes E00, E04, E09, E12,
E14 and both OPT-06 candidates. Selection requires the unchanged G1 gates and uses
the registered equal-horizon J objective.

The full OPT-06 run passed G0, including exact E00 reproduction. Both E15 and E16
passed all six G1 gates. E16 was selected by the registered J objective (`0.169902`
