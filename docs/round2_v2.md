...
重训已完成：`local/runs/round2-v2/m0-release-r1`，实现提交 `b1ac41a`。G0：锁定 Python 3.12 下 24 项相关测试通过，独立进程禁读训练数据的冷推理逐字节一致；独立 ZIP 回读确认规定三列、322 个唯一 V2 ID、与测试表同序、预测非负且有限、ZIP 根目录仅 result.csv。

G1：V2 五折 OOF（种子 42）铁量 WMAPE 18.145341%、时长 17.233698%、J 17.689520%；种子 3407 分别为 18.150510%、17.250445%、17.700477%。这是 M0 基线重训，未在 V2 上重新比较挑战者，不声称模型质量提升。平台上传 0、平台成绩为空。

提交包 ZIP SHA-256：`b4f285db9d4549d4a2ed4536358f1973bd1bdab7f75d6dabc6a76307e909f766`。原桌面旧版包未替换；应使用本次运行目录内的 V2 新包。
