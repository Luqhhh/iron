...
## 平台反馈与最终收口（2026-09-20 追加）

用户回传唯一候选 V36 的平台显示成绩为 **83.3264**。未取得 submission ID、提交时间或账号原始回执，证据等级保持 `USER_REPORTED_NOT_INDEPENDENTLY_VERIFIED`。

| 候选 | 用户回传 | 相对 V34T=83.3201 | 相对 V30A=83.3175 | 相对 V35B=83.2778 | 决策 |
| --- | ---: | ---: | ---: | ---: | --- |
| V36T_V34_V30_EQUAL_BLEND | 83.3264 | +0.0063 | +0.0089 | +0.0486 | 晋级为新最高用户回传完整包 |

V36 比端点显示值的理想未舍入均值估算 83.3188 高 `0.0076`，说明隐藏评测上的端点误差发生了足以超过较强端点的抵消；这只支持当前固定 payload，不能外推为融合必然提分。历史回放中 V36 相对 V34T 的 J 仍退化 `+0.00136040`，该离线证据原样保留，不因平台结果回写。

用户提供的唯一平台槽已消费，剩余预算 0，恢复上传预算 0；不生成第二候选、不补测 V35A，也不安排新比例。当前最高用户回传完整包更新为 V36=83.3264，ZIP SHA-256 为 `49b2e39a6019d341261221b40a58f7e773edad471896c35808b9e89551f35e80`。账号当前生效条目仍因缺少账号回执登记为未知；agent 平台上传为 0。私有反馈记录 `platform_feedback_user_reported.json` 的 SHA-256 为 `788d06a7faebf2e2f55011aaf486fb69078ea5a174fbc6cf42c5a906ac961a11`。
