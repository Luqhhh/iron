...
本文件记录提交交付和后续用户回传；阶段中的“当前”仅指记录当时。截至 2026-09-15，活动发布为 V1 = 83.0319，R2 = 83.0207 保留回退；初赛最高用户回传实验包为 V10 = 83.1951。开发失败且未提交的候选不计作平台结果；平台分数不表述为编码代理独立复现。
...
### 2026-09-09：r2 E12-raw 全量训练 test_a 用户回传 82.5430

当前桌面 ZIP 已核验为
`a713ee00f3f3a0d741199e8c271109c078312bd613a4a789a2cfb4b4128da8e6`。
按本轮唯一待测 test_a 包记录：E12-raw，2,754 行合法训练标签，
fit cutoff=2024-12-01 01:44+08:00，固定 E09/E04 80/20、800 轮、零校准。
成绩 82.5430，证据为 USER_REPORTED_NOT_INDEPENDENTLY_VERIFIED。
比 E16 82.9918 低 0.4488；比旧 development E12-raw 82.9543 低 0.4113。
同为用户报告的 test_a 成绩，未独立核验平台数据版本或回执。
该结果不支持本次全量训练提高平台成绩；不能由总分单独定位铁量、时长或历史更新的影响。
G0 工程验收与旧 G1 结论不改写，November 已消费状态保持。
本次仅记录反馈，没有新拟合、标签读取、包替换、平台上传或远端写入。
E16 归档仍可恢复；test_b/test_c 包只能用于相应阶段，不继续作为 test_a 候选。

## 2026-09-09 用户确认回退 E16

用户指示“恢复，提交推送”。r2 当前发布指针恢复 E16 原包，平台历史回传为
82.9918；被回退的 E12-raw 正式训练包及 82.5430 反馈完整保留，配置归档为
`configs/optimization_v0_3_r2/e12_final_release.yaml`。
...
## 补齐 2026-09-09 历史回传：R2 与 S1

下表补齐此前分散在阶段结果中的记录，事件顺序为 R2 → S1 → V1，不代表此次新增提交。

| 候选 | 用户回传 test_a | ZIP SHA-256 | 决策 |
| --- | ---: | --- | --- |
| v0.4 R2 | 83.0207 | `e42602d3045e43b4b49dd1e1c104aa8e5c1f29c639ff5f892b3b07434ed9bbdf` | 当时晋级，当前为 V1 回退 |
| v0.6 S1 | 82.7707 | `5d74272c9abc6d12924f2c1ad96f534a0f19042210bdf7746a27e125a7498267` | 比 R2 低 0.2500，未晋级 |

证据均为 `USER_REPORTED_NOT_INDEPENDENTLY_VERIFIED`。S1 的桌面状态后来先由
...
## 2026-09-12–13 · test_a / V8–V11 后续实验回传

以下包均由用户明确要求交付或组合，平台成绩均为
`USER_REPORTED_NOT_INDEPENDENTLY_VERIFIED`；仓库没有 submission ID、提交时间或账号原始回执。

| 候选 | 用户回传成绩 | ZIP SHA-256 | 当前解释 |
| --- | ---: | --- | --- |
| V8_QRF_TIME | 83.1636 | `409218c364dac20f59169b08f95af54c1a9e206eb19b71a6115f05ce4889b402` | 用户指定最终实验；不改写原 v0.15 G1 失败 |
| V6I_RECENCY60_IRON | 83.0634 | `7c28b6c76fe05eebf90be2560b52d03111a2d0e2d9b1b44f80fed085092e385c` | 铁量优于 V1 0.0315；用于预先指定的 V10 列组合 |
| V6T_RECENCY60_TIME | 82.9852 | `7fde26ec525b1c9fdc70b9162095f5dd38819084ccc852144ce2db48d7deb1b7` | 比 V1 低 0.0467；关闭该时长路线 |
| D1_H1_SAME_CALENDAR | 82.9993 | `ace53d1c3e5e5e1273d172f07bfec5e82a5ad3a232e5479f200c9c64baa12f44` | 保留原诊断角色，不晋级 |
| V10_V6I_IRON_V8_TIME | **83.1951** | `be83f1f623c12f4ef03479f2fcacdfb756920d41c4e09d1c4f6b55a2279761ac` | 当前最高用户回传；V6I 铁量列与 V8 时长列零拟合组合 |
| V11_V6I_IRON_QRF_MEAN_TIME | 83.1806 | `f11dc1216c4c0742955a7222b16d65bc9ca4c1fd3b11d1c38fb7416f920b5efa` | 比 V10 低 0.0145；`CLOSE_V11_RETAIN_V10` |

V8 独立交付消费 1 个最终 forest 和 1 个 preprocessor；V6I/V6T/D1 回收实验合计消费
2 次最终单目标 CatBoost 与 3 次标量 LAD。V10 和 V11 均为已有预测的零拟合组合/探针，
未读取新官方标签，编码代理上传次数为 0。所有原开发 G1、V1 活动发布登记和 R2 回退登记保持不变；
V11 回传后本轮探针关闭，v0.16 继续暂停。
